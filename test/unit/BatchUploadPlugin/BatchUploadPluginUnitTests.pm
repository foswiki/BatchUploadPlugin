...
sub new {
    ...
    $this->{attachmentDir} = "$Foswiki::cfg{WorkingDir}/../../BatchUploadPlugin/test/unit/BatchUploadPlugin/attachment_examples/";
    if (! -e $this->{attachmentDir}) {
        die "Can't find attachment_examples directory (tried $this->{attachmentDir})";
    }
...
sub test_openZipSanityCheck {
    ...
    my $zip = Foswiki::Plugins::BatchUploadPlugin::openZipSanityCheck(
        $fh,
        $this->{test_web},
        $this->{test_topic},
        'archive.zip'
...
sub test_isZip {
    ...
    $this->assert(
        Foswiki::Plugins::BatchUploadPlugin::isZip('foo.zip')
    );
    $this->assert(!
        Foswiki::Plugins::BatchUploadPlugin::isZip('foo.docx')
    );
}
